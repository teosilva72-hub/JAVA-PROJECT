...
public class OccurencesDao2{
	...
	public String cadastroOcorrencia (OccurenceData2 data ) throws Exception {
		...
		//script BD
		String query =  "INSERT INTO occ_data2 (date, hora, pedagio, folio, report, sinistro, direcao, kmregistro, kminicial, kmfinal, horaregistro, horachega, politica,  tipoveic, quantidade, marca, tipo_veic, modelo, color, placa, telefone, nome, idade, saude, motivo, observacao)"
				+  " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
		...
		try {
			...
			ps.setString(16, data.getMarca());	
			ps.setString(17, data.getTipo());
			ps.setString(18, data.getModelo());
			ps.setString(19, data.getCor());
			ps.setString(20, data.getPlaca());
			ps.setString(21, data.getTelefone());
			ps.setString(22, data.getNome());
			ps.setString(23, data.getIdade());
			ps.setString(24, data.getSaude());
			ps.setString(25, data.getMotivo());
