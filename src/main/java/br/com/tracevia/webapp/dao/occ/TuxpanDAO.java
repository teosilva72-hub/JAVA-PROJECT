...
public class TuxpanDAO{
	...
	public boolean update(TuxpanOccModel data, int id, String val) throws Exception {
		boolean check = false;
		String query = "";
		
		if(val.equals("1"))
			query = "UPDATE occ_accidentes SET cobro = ?, folio_secuencial = ?, reporte = ?, siniestro = ?, fecha = ?, direccion = ?, km_reg = ?, km_inicial = ?, km_final = ?, poliza = ?,"+
			...
			query = "UPDATE occ_preliminar SET cobro = ?, folio_secuencial = ?, reporte = ?, siniestro = ?, fecha = ?, direccion = ?, km_reg = ?, km_inicial = ?, km_final = ?, poliza = ?,"+
					"hora_reg_cab = ?, hora_arr_aju = ?, semoviente = ?, trabajo_cons = ?, tiempo = ?, neblina = ?, vandalismo = ?, otro = ?, obs = ?, tipo_veh = ?,"+
					"tipo_veh_eje = ?, num_veh_inv = ?, marca_veh_inv = ?, tipo_veh_inv = ?, modelo_veh_inv = ?, color_veh_inv = ?, placa_est_veh_inv = ?, tel_veh_inv = ?, num_person = ?, nombre_person = ?,"+
					"edad_person = ?, condiccion_person = ?, hora = ?, lesionado = ?, causas = ?, ocupantes_vh = ?, vh_ocupantes = ?, def_metal = ?, sinal = ?, dano_pav = ?, dano_cor_ter = ?, dano_obr = ?,"+
					"dano_plz = ?, otros_sin = ?, muerto = ?, obs_sin = ? WHERE id ="+id+"";
	...
	public TuxpanOccModel selectAccidente(String id) throws Exception {
		...
		String query = "SELECT cobro, folio_secuencial, reporte, siniestro, fecha, direccion, km_reg, km_inicial, km_final, poliza,"+
				...
				"def_metal, sinal, dano_pav, dano_cor_ter, dano_obr, dano_plz, otros_sin, muerto, obs_sin FROM occ_accidentes WHERE id ='"+id+"'";
		try {
			conn.start(1);
			conn.prepare(query);
	...
	public TuxpanOccModel selectPreliminar(String id) throws Exception {
		TuxpanOccModel report = new TuxpanOccModel();
		String query = "SELECT cobro, folio_secuencial, reporte, siniestro, fecha, direccion, km_reg, km_inicial, km_final, poliza,"+
				"hora_reg_cab, hora_arr_aju, semoviente, trabajo_cons, tiempo, neblina, vandalismo, otro, obs, tipo_veh, tipo_veh_eje,"+
				"num_veh_inv, marca_veh_inv, tipo_veh_inv, modelo_veh_inv, color_veh_inv, placa_est_veh_inv, tel_veh_inv, num_person,"+
				"nombre_person, edad_person, condiccion_person, hora, type_report, id, lesionado, causas, ocupantes_vh, vh_ocupantes,"+
				"def_metal, sinal, dano_pav, dano_cor_ter, dano_obr, dano_plz, otros_sin, muerto, obs_sin, idPasta FROM occ_preliminar WHERE id ='"+id+"'";
		try {
			conn.start(1);
			conn.prepare(query);
			MapResult result = conn.executeQuery();
			if(result.hasNext()) {
				for (RowResult rs : result) {
					...
					report.setMortos(rs.getString(47));
					report.setObs_sin(rs.getString(48));
					report.setIdPasta(rs.getString(49));
	...
	public boolean deleteRegister(String id, String type) throws Exception {
		boolean check = false;
		String query = "";
		
		if(type.equals("1"))
			query = "DELETE FROM occ_accidentes WHERE id = '"+id+"'";
			
		else if(type.equals("2"))
			query = "DELETE FROM occ_preliminar WHERE id = '"+id+"'";
	...
	public boolean registerOcc(TuxpanOccModel data, String type_report, String idPasta) throws Exception {
		boolean saveOcc = false;
		String query = "";
		if(type_report.equals("1"))
			query = "INSERT INTO occ_accidentes(cobro, folio_secuencial, reporte, siniestro, fecha, direccion, km_reg, km_inicial, km_final, poliza,"+
					"hora_reg_cab, hora_arr_aju, semoviente, trabajo_cons, tiempo, neblina, vandalismo, otro, obs, tipo_veh, tipo_veh_eje,"+
					"num_veh_inv, marca_veh_inv, tipo_veh_inv, modelo_veh_inv, color_veh_inv, placa_est_veh_inv, tel_veh_inv, num_person,"+
					...
					" VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		else if(type_report.equals("2"))
			query = "INSERT INTO occ_preliminar(cobro, folio_secuencial, reporte, siniestro, fecha, direccion, km_reg, km_inicial, km_final, poliza,"+
					"hora_reg_cab, hora_arr_aju, semoviente, trabajo_cons, tiempo, neblina, vandalismo, otro, obs, tipo_veh, tipo_veh_eje,"+
					"num_veh_inv, marca_veh_inv, tipo_veh_inv, modelo_veh_inv, color_veh_inv, placa_est_veh_inv, tel_veh_inv, num_person,"+
					"nombre_person, edad_person, condiccion_person, hora, type_report, "+
					"def_metal, sinal, dano_pav, dano_cor_ter, dano_obr, dano_plz, otros_sin, ocupantes_vh, vh_ocupantes, lesionado, muerto, causas, obs_sin, idPasta)"+
					" VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	public ArrayList<TuxpanOccModel> listAccidentes() throws Exception {
		String query = "SELECT id, folio_secuencial, reporte, siniestro, fecha, hora, type_report FROM occ_accidentes";	
	...
	public ArrayList<TuxpanOccModel> listPreliminar() throws Exception {
		String query = "SELECT id, folio_secuencial, reporte, siniestro, fecha, hora, type_report FROM occ_preliminar";	
