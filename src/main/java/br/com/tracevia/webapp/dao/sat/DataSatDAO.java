...
public class DataSatDAO {
	...
	public List<SAT> dataInterval(ListEquipments equips, String interval, int time) throws Exception {
		...
		DateTimeApplication dta = new DateTimeApplication();
			...
			int periodInterval = 4; // Number os events per hour (60 min / 15 min)
			...
			"ELSE CONCAT(DATE_FORMAT(d.DATA_HORA, '%d/%m/%y %H:%i -'), DATE_FORMAT(DATE_ADD(d.DATA_HORA, INTERVAL 15 MINUTE), ' %H:%i')) END 'INFO_HEADER', " +
			...
			"CASE WHEN eq.number_lanes = 2 THEN IFNULL(ROUND(AVG(NULLIF(IF(d.NOME_FAIXA = 2, d.VEL_MEDIA_AUTOS , NULL),0)), 0), 0) WHEN eq.number_lanes = 3 THEN IFNULL(ROUND(AVG(NULLIF(IF(d.NOME_FAIXA > 1 AND (eq.dir_lane1 <> eq.dir_lane2), d.VEL_MEDIA_AUTOS , NULL),0)), 0), 0) WHEN eq.number_lanes = 4 THEN IFNULL(ROUND(AVG(NULLIF(IF(d.NOME_FAIXA > 2, d.VEL_MEDIA_AUTOS , NULL ),0)), 0), 0) WHEN eq.number_lanes = 5 THEN IFNULL(ROUND(AVG(NULLIF(IF(d.NOME_FAIXA > 3 AND (eq.dir_lane1 <> eq.dir_lane4), d.VEL_MEDIA_AUTOS , NULL ),0)), 0), 0) WHEN eq.number_lanes = 6 THEN IFNULL(ROUND(AVG(NULLIF(IF(d.NOME_FAIXA > 3, d.VEL_MEDIA_AUTOS, NULL),0)), 0), 0) " +
				 "WHEN eq.number_lanes = 7 THEN IFNULL(ROUND(AVG(NULLIF(IF(d.NOME_FAIXA > 4 AND (eq.dir_lane1 <> eq.dir_lane5), d.VEL_MEDIA_AUTOS , NULL ),0)), 0), 0) WHEN eq.number_lanes = 8 THEN IFNULL(ROUND(AVG(NULLIF(IF(d.NOME_FAIXA > 4, d.VEL_MEDIA_AUTOS , NULL ),0)), 0), 0) ELSE 0 END 'VEL_MEDIA_AUTO_S1', " +
			...
			"CASE WHEN eq.number_lanes = 2 THEN IFNULL(ROUND(AVG(NULLIF(IF(d.NOME_FAIXA = 2, d.VEL_MEDIA_AUTOS , NULL),0)), 0), 0) WHEN eq.number_lanes = 3 THEN IFNULL(ROUND(AVG(NULLIF(IF(d.NOME_FAIXA > 1 AND (eq.dir_lane1 <> eq.dir_lane2), d.VEL_MEDIA_AUTOS , NULL),0)), 0), 0) WHEN eq.number_lanes = 4 THEN IFNULL(ROUND(AVG(NULLIF(IF(d.NOME_FAIXA > 2, d.VEL_MEDIA_AUTOS , NULL ),0)), 0), 0) WHEN eq.number_lanes = 5 THEN IFNULL(ROUND(AVG(NULLIF(IF(d.NOME_FAIXA > 3 AND (eq.dir_lane1 <> eq.dir_lane4), d.VEL_MEDIA_AUTOS , NULL ),0)), 0), 0) WHEN eq.number_lanes = 6 THEN IFNULL(ROUND(AVG(NULLIF(IF(d.NOME_FAIXA > 3, d.VEL_MEDIA_AUTOS, NULL),0)), 0), 0) " +
				 "WHEN eq.number_lanes = 7 THEN IFNULL(ROUND(AVG(NULLIF(IF(d.NOME_FAIXA > 4 AND (eq.dir_lane1 <> eq.dir_lane5), d.VEL_MEDIA_AUTOS , NULL ),0)), 0), 0) WHEN eq.number_lanes = 8 THEN IFNULL(ROUND(AVG(NULLIF(IF(d.NOME_FAIXA > 4, d.VEL_MEDIA_AUTOS , NULL ),0)), 0), 0) ELSE 0 END 'VEL_MEDIA_AUTO_S2', " +
			...
			"CASE WHEN eq.number_lanes = 2 THEN IFNULL(ROUND(AVG(NULLIF(IF(d.NOME_FAIXA = 2, d.VEL_MEDIA_MOTOS , NULL),0)), 0), 0) WHEN eq.number_lanes = 3 THEN IFNULL(ROUND(AVG(NULLIF(IF(d.NOME_FAIXA > 1 AND (eq.dir_lane1 <> eq.dir_lane2), d.VEL_MEDIA_MOTOS , NULL),0)), 0), 0) WHEN eq.number_lanes = 4 THEN IFNULL(ROUND(AVG(NULLIF(IF(d.NOME_FAIXA > 2, d.VEL_MEDIA_MOTOS , NULL ),0)), 0), 0) WHEN eq.number_lanes = 5 THEN IFNULL(ROUND(AVG(NULLIF(IF(d.NOME_FAIXA > 3 AND (eq.dir_lane1 <> eq.dir_lane4), d.VEL_MEDIA_MOTOS , NULL ),0)), 0), 0) WHEN eq.number_lanes = 6 THEN IFNULL(ROUND(AVG(NULLIF(IF(d.NOME_FAIXA > 3, d.VEL_MEDIA_MOTOS, NULL),0)), 0), 0) " +
				 "WHEN eq.number_lanes = 7 THEN IFNULL(ROUND(AVG(NULLIF(IF(d.NOME_FAIXA > 4 AND (eq.dir_lane1 <> eq.dir_lane5), d.VEL_MEDIA_MOTOS , NULL ),0)), 0), 0) WHEN eq.number_lanes = 8 THEN IFNULL(ROUND(AVG(NULLIF(IF(d.NOME_FAIXA > 4, d.VEL_MEDIA_MOTOS , NULL ),0)), 0), 0) ELSE 0 END 'VEL_MEDIA_MOTO_S2', " +
		 ...
		 conn.start(1);
			...
			if (result.hasNext()) {
				for (RowResult rs : result) {
					...
					sat.setCurrentDatetime(rs.getString("INFO_HEADER")); //  MAIN HEADER					
					...
					sat.setAutosProjection1hS1(sat.getAutosVolumeS1() * periodInterval);
					sat.setComProjection1hS1(sat.getComVolumeS1() * periodInterval);
					sat.setMotoProjection1hS1(sat.getMotoVolumeS1() * periodInterval);
					sat.setTotalProjection1hS1(sat.getTotalVolumeS1() * periodInterval);
					...
					sat.setAutosProjection1hS2(sat.getAutosVolumeS2() * periodInterval);
					sat.setComProjection1hS2(sat.getComVolumeS2() * periodInterval);
					sat.setMotoProjection1hS2(sat.getMotoVolumeS2() * periodInterval);
					sat.setTotalProjection1hS2(sat.getTotalVolumeS2() * periodInterval);
		...
		hourDatetime = dta.getDataIntervalHour(calendar);
 		...
 		String select = "SELECT d.NOME_ESTACAO AS ESTACAO, CASE WHEN DATEDIFF(NOW(), d.DATA_HORA) > 0 THEN date_format(d.DATA_HORA, '%d/%m/%y %H:%i') ELSE CASE WHEN MINUTE(d.DATA_HORA) = 45 THEN CONCAT(DATE_FORMAT(d.DATA_HORA, '%H:%i -'), " + 
 		 ...
 		 conn.start(1);
 			...
 			conn.setString(3, hourDatetime);
 			conn.setString(4, hourDatetime);
 			conn.setString(5, hourDatetime);
 			conn.setString(6, hourDatetime); 			
 			conn.setString(7, currentDate);
 			conn.setString(8, currentDate);
 						
 			MapResult result = conn.executeQuery();
 			
 		//	 System.out.println("EQUIP: "+select);
 ...
 // -------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  public String[] lastRegisters(int equip) throws Exception {
 		...
 		String select = "SELECT CASE WHEN DATEDIFF(NOW(), d.DATA_HORA) > 0 THEN date_format(d.DATA_HORA, '%d/%m/%y %H:%i') ELSE " + 			
 						"CASE WHEN MINUTE(d.DATA_HORA) = 45 THEN CONCAT(DATE_FORMAT(d.DATA_HORA, '%H:%i -'), DATE_FORMAT(DATE_ADD(d.DATA_HORA ,INTERVAL 14 MINUTE), ' %H:%i')) " +
 						"ELSE CONCAT(DATE_FORMAT(d.DATA_HORA, '%H:%i -'), DATE_FORMAT(DATE_ADD(d.DATA_HORA, INTERVAL 15 MINUTE), ' %H:%i')) END END 'PACOTE_HORA', " + 
 						"CASE WHEN DATEDIFF(NOW(), md.maxDate) > 0 THEN date_format(md.maxDate, '%d/%m/%y %H:%i') ELSE date_format(md.maxDate, '%H:%i') END 'DADO_HORA' " +
